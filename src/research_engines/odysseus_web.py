"""Bridge Odysseus ``services/search`` to research finding dicts (Phase L1 Tier A)."""
...
def search_via_odysseus_web(
    ...
    *,
    ...
    """Run a web search through the shared Odysseus provider layer."""
    ...
    try:
        ...
        logger.info("Odysseus web search failed for %r: %s", query[:80], e)
        return []

    if outcome.error and not outcome.results:
        logger.info("Odysseus web search error: %s", outcome.error)
        return []
