...
def _ollama_normalize_tool_messages(messages: List[Dict]) -> List[Dict]:
    """Adapt Nobody' canonical OpenAI-style messages to native Ollama /api/chat.

    Nobody carries assistant tool calls in the OpenAI shape, where
...
def _provider_headers(provider: str, headers: Optional[Dict] = None) -> Dict[str, str]:
    ...
    if provider == "openrouter":
        h.setdefault("HTTP-Referer", "https://github.com/myfacekid/Nobody")
        h.setdefault("X-OpenRouter-Title", "Nobody")
    return h
...
def _sanitize_llm_messages(messages: List[Dict]) -> List[Dict]:
    """Strip Nobody-only metadata before sending messages to providers.
                    ...
                    try:
                        if data.strip():
                            if data.startswith("{"):
                                ...
                                # DeepSeek (and some gateways) emit choices: [null]
                                # on keepalive / usage-only chunks. Treat that as
                                # an empty choice so we don't blow up on .get().
                                if not isinstance(_choice0, dict):
                                    _choice0 = {}
                                _delta0 = _choice0.get("delta")
