...
async def do_ui_control(content: str, session_id: Optional[str] = None) -> Dict:
    ...
    Actions:
      ...
      set_mode <agent|plan|chat>   — Switch between agent, plan, and chat mode
    ...
    if action == "toggle":
        ...
        if len(parts) < 2:
            return {"error": "set_mode needs: set_mode <agent|plan|chat>"}
        mode = parts[1].lower()
        if mode not in ("agent", "plan", "chat"):
            return {"error": f"Invalid mode '{mode}'. Use: agent, plan, chat"}
        return {
            "ui_event": "set_mode",
            "mode": mode,
