#!/bin/bash
# Nobody — one-command quick start for macOS (Apple Silicon).
#
#   ./start-macos.sh
#
# Installs everything Nobody needs via Homebrew, sets up a local Python
...
# that's already done.
#
# Why native (not Docker): Cookbook serves models on whatever machine Nobody
...
# Shell overrides (NOBODY_PORT / NOBODY_HOST) take top priority, then .env
# values (APP_PORT / APP_BIND), then built-in defaults.
PORT="${NOBODY_PORT:-${APP_PORT:-7860}}"   # 7860, not 7000 — macOS AirPlay Receiver holds 7000.
HOST="${NOBODY_HOST:-${APP_BIND:-127.0.0.1}}" # Set APP_BIND=0.0.0.0 in .env for LAN/Tailscale access.
PROBE_HOST="$HOST"
...
echo "▶ Nobody quick start for macOS"
...
if (exec 3<>"/dev/tcp/$PROBE_HOST/$PORT") 2>/dev/null; then
  ...
  echo "    NOBODY_PORT=7900 ./start-macos.sh"
  exit 1
fi
...
# 2. Find a Python 3.12–3.13 to build the environment with.
...
#    (or non-mac) we just use whatever Python 3.12–3.13 is on PATH.
#    Deep Research (local-deep-research) needs 3.12–3.13; 3.14+ lacks wheels.
PY=""
if [ "$(uname -m)" = "arm64" ]; then
  cands="/opt/homebrew/bin/python3.13 /opt/homebrew/bin/python3.12"
else
  cands="python3.13 python3.12 python3"
fi
for cand in $cands; do
  p="$(command -v "$cand" 2>/dev/null)" || continue
  if "$p" -c 'import sys; raise SystemExit(0 if (3, 12) <= sys.version_info[:2] <= (3, 13) else 1)' 2>/dev/null; then
    PY="$p"; break
  fi
done
...
if [ -z "$PY" ] || [ ! -x "$PY" ]; then
  echo "✗ Couldn't find a Python 3.12–3.13 to build the environment with."
...
echo "▶ Preparing Nobody…"
NOBODY_SKIP_RUN_HINT=1 ./venv/bin/python setup.py
...
# NOBODY_NO_OPEN=1 (e.g. over SSH / headless).
POLLER_PID=""
if [ -z "$NOBODY_NO_OPEN" ] && command -v open >/dev/null 2>&1; then
  (
    for _ in $(seq 1 90); do
      if (exec 3<>"/dev/tcp/$PROBE_HOST/$PORT") 2>/dev/null; then
        printf '\n'
        printf '  ┌────────────────────────────────────────────┐\n'
        printf '  │  ✓ Nobody is ready — opening your browser  │\n'
        printf '  │     %-40s │\n' "$URL"
...
echo
echo "▶ Starting Nobody — it will open in your browser at $URL"
