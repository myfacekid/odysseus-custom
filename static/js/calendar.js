...
function _showEventMoreMenu(ev, anchor) {
  ...
  dropdown.style.cssText = `position:fixed;z-index:10001;min-width:180px;background:var(--panel,var(--bg));border:1px solid var(--border);border-radius:2px;box-shadow:2px 2px 0 color-mix(in srgb, var(--fg) 18%, transparent);padding:4px;font-size:12px;top:${rect.bottom + 4}px;left:0px;visibility:hidden;`;
...
const BADGE_SEEN_KEY = 'nobody-calendar-badge-seen';
...
function _headerHTML() {
  ...
  return `<div class="cal-toolbar">
    ...
    <div class="cal-toolbar-right">
      ...
      ${_filtersToggleHTML()}
      <button class="cal-add-btn cal-add-btn-text" id="cal-add" title="New event"><span class="cal-add-plus">+</span><span class="cal-add-label">Add event</span></button>
    </div>
  </div>
...
function _filtersData() {
  ...
  if (_calendars.length > 1) {
    calFilters = _calendars.map(c => {
      ...
      return `<label class="cal-filter-item notes-label-chip${off ? ' cal-filter-off' : ''}" data-href="${_e(c.href)}">
  ...
  for (const t of typeOrder) {
    ...
    typeFilters += `<label class="cal-filter-item notes-label-chip${off ? ' cal-filter-off' : ''}${active ? ' active cal-filter-active' : ''}${t === '!' ? ' cal-filter-important' : ''}" data-type="${t}">
      <span class="cal-filter-dot" style="background:${_TYPE_PALETTE[t]}"></span>${label}</label>`;
  }
  if (hasUntagged) {
    const off = _hiddenTypes.has('__untagged__');
    typeFilters += `<label class="cal-filter-item notes-label-chip${off ? ' cal-filter-off' : ''}" data-type="__untagged__">
...
function _filtersToggleHTML() {
  ...
  return `<button class="cal-filter-toggle notes-label-chip" id="cal-filter-toggle" title="${_filtersCollapsed ? 'Show filters' : 'Hide filters'}">${_filtersCollapsed ? '+ tags' : '− tags'}</button>`;
}
...
function _wireAll(body) {
  ...
  try {
    ...
    if (calBody && splitter) {
      ...
      if (!alreadySet) {
        const saved = parseInt(localStorage.getItem('nobody.cal.detailH') || '0', 10);
      ...
      const onUp = () => {
        ...
        if (px) { try { localStorage.setItem('nobody.cal.detailH', String(px)); } catch {} }
      };
      const onDown = (ev) => {
        ev.preventDefault();
        dragging = true;
      ...
      const resetSplit = () => {
        ...
        try { localStorage.removeItem('nobody.cal.detailH'); } catch {}
      };
      ...
      <div class="modal-body" style="padding:16px;display:flex;flex-direction:column;gap:16px;">
        <div>
          ...
          <div id="cal-settings-list" style="display:flex;flex-direction:column;gap:4px;">
            ${cals.map(c => `
              <div class="cal-settings-row" data-id="${_e(c.href)}" style="display:flex;align-items:center;gap:8px;padding:6px 8px;border-radius:2px;background:color-mix(in srgb, var(--fg) 4%, transparent);">
                ...
                <input type="text" value="${_e(c.name)}" class="cal-s-name" style="flex:1;background:none;border:1px solid var(--border);border-radius:2px;padding:3px 6px;color:var(--fg);font-size:12px;" />
...
const LS_KEY = 'nobody-calendar-cache';
