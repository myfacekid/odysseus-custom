...
import * as Modals from './modalManager.js';
  ...
  // Inject tab menu styles immediately
  {
  ...
  // Email compose removed — no-op stubs for legacy callers.
  function _parseEmailHeader(content) {
    return { to: '', cc: '', bcc: '', subject: '', inReplyTo: '', references: '', sourceUid: '', sourceFolder: '', attachments: [], body: content || '' };
  }
  function _buildEmailContent(_to, _subject, _inReplyTo, _references, body) { return body || ''; }
  function _hideEmailFields() {}
  function _showEmailFields() {}
  function _emailRichbodyActive() { return null; }
  function _syncEmailRichbody() {}
  async function _streamEmailBodyText(textarea, value) { if (textarea) textarea.value = value || ''; }
  function _emailReplyOwnText(text) { return (text || '').trim(); }
  ...
  export function openPanel() {
    ...
    function _applyDocFont() {
      if (editorWrap) {
        ...
        if (_fontSizes[_fontIdx] !== 's') editorWrap.classList.add('doc-font-' + _fontSizes[_fontIdx]);
      }
      if (fontBtn) {
   ...
   */
  async function _sendSignedReply(_docId) {
    if (uiModule) uiModule.showError('Signed email replies are no longer available (email was removed). Export the PDF instead.');
  }
