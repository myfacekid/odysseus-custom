...
const RECENT_KEY = 'nobody-model-recent';
...
function _initModelPickerDropdown() {
  ...
  const _collapsedProviders = new Set(_loadList('nobody-model-collapsed'));
  ...
  function _populate(filter) {
    ...
    function _addRow(m) {
      ...
      if (m.stale) {
        ...
        badge.style.cssText = 'font-size:10px;opacity:0.7;padding:1px 6px;border:1px solid var(--border);border-radius:2px;margin-left:6px;';
    ...
    if (all.length <= BROWSE_ALL_LIMIT) {
      ...
      sorted.forEach(provider => {
        ...
        header.addEventListener('click', (e) => {
          ...
          _saveList('nobody-model-collapsed', [..._collapsedProviders]);
          const st = listEl.scrollTop;
          _populate('');
          listEl.scrollTop = st;
        });
  ...
  async function _pick(m) {
    ...
    try { document.dispatchEvent(new CustomEvent('nobody:model-picked', { detail: m })); } catch {}
  ...
  document.addEventListener('nobody:auto-select-model', async (e) => {
...
export function updateModelPicker() {
  ...
  // SECURITY: deliberately NOT auto-injecting `nobody-model-favorites[0]`
