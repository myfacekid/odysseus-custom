    ...
    data.users.forEach(u => {
      ...
      if (!u.is_admin) {
        ...
        html += `<div style="display:flex;align-items:center;justify-content:space-between;padding:4px 0;">
          ...
          <input type="number" min="0" value="${maxMsg}" data-priv="max_messages_per_day" data-user="${esc(u.username)}" style="width:70px;padding:4px 6px;background:var(--bg);border:1px solid var(--border);border-radius:2px;color:var(--fg);font-size:12px;text-align:center;">
        </div>`;
...
async function _selectAddedModelInChat(endpoint) {
  ...
  try {
    document.dispatchEvent(new CustomEvent('nobody:auto-select-model', {
      detail: {
...
function initEndpointForm() {
  ...
  document.querySelectorAll('#adm-add-api, #adm-add-local').forEach((sec) => {
    ...
    if (!head) return;
    const key = 'nobody.addModels.' + sec.id + '.open';
    let open = false;
    ...
    list.innerHTML = servers.map(s => {
      ...
      return `<div class="admin-user-row" data-adm-mcp-id="${s.id}">
        <div style="display:flex;align-items:center;justify-content:space-between;${hasTools ? 'cursor:pointer;' : ''}padding:4px 0;" data-adm-mcp-header="${s.id}">
          ...
          <div style="display:flex;gap:4px;align-items:center;">
            ${s.needs_oauth ? `<a href="/api/mcp/oauth/authorize/${s.id}" target="_blank" class="admin-btn-sm" style="background:var(--red);color:#fff;text-decoration:none;padding:3px 10px;border-radius:2px;font-size:11px;font-weight:600;">Authorize</a>` : ''}
...
function initMcpForm() {
  ...
  function _buildEnvFields(envObj, help, preset) {
    ...
    if (preset?.providerDropdown) {
      ...
      select.style.cssText = 'flex:1;padding:6px 8px;border-radius:2px;border:1px solid var(--border);background:var(--bg-secondary);color:var(--text-primary);font-size:12px;';
...
function initBackup() {
  el('adm-exportDataBtn').addEventListener('click', async () => {
    ...
    try {
      ...
      const filename = match ? match[1] : 'nobody_backup.json';
