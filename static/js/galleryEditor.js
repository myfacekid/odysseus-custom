...
import { observeGeFloaters } from './geTileWindows.js';
...
// Async check whether `rembg` is installed on the Nobody server.
...
function _saveProject() {
  ...
  const project = {
    v: 1,
    type: 'nobody-gallery-editor-project',
    imgWidth: state.imgWidth,
    imgHeight: state.imgHeight,
    activeLayerId: state.activeLayerId,
    nextLayerId: state.nextLayerId,
...
function _loadProjectPrompt() {
  ...
  inp.addEventListener('change', async () => {
    ...
    try {
      ...
      if (proj.type !== 'nobody-gallery-editor-project') {
...
export function openEditor(imageUrl, imageId, presetSize, displayName, draftId) {
  ...
  if (!state._geTileObsDisconnect) {
    state._geTileObsDisconnect = observeGeFloaters(state.container);
  }
