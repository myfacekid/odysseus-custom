...
import { promoteGeFloater } from '../../geTileWindows.js';
import {
  ADJ_ICONS,
  adjLayerLabel,
  defaultAdjParams,
...
export function createAdjPopupSystem({ composite, saveState, renderLayerPanel }) {
  ...
  function openAdjPopup(layer, type, anchorEl, existingAdj) {
    ...
    // Desktop: makeWindowDraggable (via promoteGeFloater) owns the drag.
    // Mobile keeps the lightweight pointer handler below.
    if (head && pop.dataset.geTileWired !== '1') {
      const isMobile = window.matchMedia('(max-width: 820px)').matches;
      const setPos = (x, y) => {
        if (isMobile) {
