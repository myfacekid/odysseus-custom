...
DOCKER_IN_CONTAINER_HINT = (
    "Not available inside the Nobody container by design. The image ships no "
...
TMUX_LOG_DIR = Path(tempfile.gettempdir()) / "nobody-tmux"
...
async def _generate_tmux(cmd: str, request: Request):
    ...
    script_path.write_text(
        f"#!/bin/bash\n"
        f"NOBODY_USER_SHELL=\"${{SHELL:-}}\"\n"
        f"if [ -n \"$NOBODY_USER_SHELL\" ] && [ -x \"$NOBODY_USER_SHELL\" ]; then\n"
        f"  NOBODY_USER_PATH=\"$(\"$NOBODY_USER_SHELL\" -ic 'printf \"__NOBODY_PATH__%s\\n\" \"$PATH\"' 2>/dev/null | sed -n 's/^__NOBODY_PATH__//p' | tail -n 1 || true)\"\n"
        f"  if [ -n \"$NOBODY_USER_PATH\" ]; then export PATH=\"$NOBODY_USER_PATH:$PATH\"; fi\n"
        f"fi\n"
...
def setup_shell_routes() -> APIRouter:
    ...
    async def list_packages(request: Request, host: str | None = None, ssh_port: str | None = None, venv: str | None = None):
        ...
        packages = [
            ...
            # ── Image ── diffusion model serving (txt2img)
    ...
    async def install_package(request: Request):
        ...
        known = {
            ...
            "TTS", "bark", "faster-whisper", "playwright",
