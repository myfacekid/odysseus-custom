...
def create_default_admin():
    ...
    try:
        ...
        def _env(name: str) -> str:
            return os.environ.get(f"NOBODY_{name}", "")

        username = _env("ADMIN_USER").strip().lower()
        password = _env("ADMIN_PASSWORD").strip()
        admin_password_set = bool(_env("ADMIN_PASSWORD"))

        if username and password:
            # Both provided via env — use them directly
            pass
        elif sys.stdin.isatty() and not _env("SKIP_ADMIN_PROMPT"):
        ...
        if sys.stdin.isatty() and not admin_password_set:
            print(f"  [ok] Admin account created ({username})")
        else:
            print(f"  [ok] Initial admin user created ({username})")
            if not admin_password_set:
                print(f"        Temporary password: {password}")
                print(f"        ** Change it after first login. Set NOBODY_ADMIN_PASSWORD to choose your own. **")
        return "created"
    except ImportError:
...
def main():
    ...
    if not os.environ.get("NOBODY_SKIP_RUN_HINT"):
