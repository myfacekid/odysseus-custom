...
and run alongside Nobody on `docker compose up`. They are not modified —
just composed.
...
Nobody talks to these over the network/API. They are **not** distributed
...
- **`caldav`** (Python lib) is **dual-licensed GPL-3.0-or-later OR Apache-2.0**.
  Nobody uses it under **Apache-2.0**, which is permissive and MIT-compatible.
...
Most of Nobody's code was written *with* AI models, not just by a human.
